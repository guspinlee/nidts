...
impl Polynomial {
    ...
    */
    ...
    /// Performs polynomial multi-eval using a pre-computed sub-product tree of xs.
    /// `tree` must be the same as the result when calling SubProductTree::new_from_points(xs)
    pub fn multi_eval_with_tree(&self, xs: &[Scalar], tree: &SubProductTree) -> Vec<Scalar> {
        assert!(xs.len() > self.degree());
        tree.eval(xs.as_ref(), self)
    }
    ...
    pub fn lagrange_coefficients(xs: &[Scalar]) -> Vec<Scalar> {
        ...
        let vanishing_at_0 = tree.product.eval(&zero); //V_T(0)
        //let V_{T \ {j}}(0) = V_T(0) / (0 - j)
        let nums: Vec<Scalar> = xs
            .iter()
            .map(|j| vanishing_at_0.clone() * (- j).invert().unwrap())
            .collect();
        ...
        let cs: Vec<Scalar> = m_prime
            ...
            .iter()
            .enumerate()
            .map(|(i, c)| nums[i] * c.invert().unwrap())
            .collect();

        cs
    }

    pub fn lagrange_coefficients_naive(xs: &[Scalar], x: &Scalar) -> Vec<Scalar> {
        assert!(xs.len() > 1); // undefined for 1 point

        let mut output = Vec::new();
        for (i,x_i) in xs.iter().enumerate() {
            let mut l_i = Scalar::from(1);
            for (j, x_j) in xs.iter().enumerate() {
                if i != j {
                    let numerator = x - x_j;
                    let denominator = x_i - x_j;
                    l_i *= numerator * denominator.invert().unwrap();
                }
            }
            output.push(l_i);
        }
    ...
    pub fn lagrange_coefficients_barycentric_init(xs: &[Scalar]) -> Vec<Scalar> {
        assert!(xs.len() > 1); // undefined for 1 point

        // baryw_i = prod_{j in [n]\{i} } (x_i-x_j)^{-1}
        let mut baryws = Vec::new();
        for (i,x_i) in xs.iter().enumerate() {
            let mut bw_i = Scalar::from(1);
            for (j, x_j) in xs.iter().enumerate() {
                if i != j {
                    let denom = x_i - x_j;
                    bw_i *= denom.invert().unwrap();
                }
            }
            baryws.push(bw_i);
        }
    ...
    pub fn lagrange_coefficients_barycentric(baryws: &Vec<Scalar>, xs: &[Scalar], x: &Scalar) -> Vec<Scalar> {
        assert!(xs.len() > 1); // undefined for 1 point

        // compol = prod_{} (x-x_j)
        let mut compol = Scalar::from(1);
        for (_j, x_j) in xs.iter().enumerate() {
            let numerator = x - x_j;
            compol *= numerator;
        }

        // coeffs = { L_i(x) = common * baryws[i] / (x-x_i) }
        let mut coeffs = Vec::new();
        for (i,x_i) in xs.iter().enumerate() {
            let denom = x - x_i;
            let l_i = compol * baryws[i] * denom.invert().unwrap();
            coeffs.push(l_i);
        }
        coeffs
    }
...
mod tests {
    ...
    #[test]
    fn test_fast_multi_eval_precomputed_tree() {
        let polynomial = Polynomial::new(
            vec![2, 5, 7, 90, 111]
                .into_iter()
                .map(|x| x.into())
        ...
        let mut fast = polynomial.multi_eval_with_tree(xs.as_slice(), &tree);
        fast.truncate(xs.len());
        let mut slow = Vec::new();
    ...
    #[test]
    fn test_interpolation_precomputed_tree() {
        let xs: Vec<Scalar> = vec![2].into_iter().map(|x| x.into()).collect();
        let ys: Vec<Scalar> = vec![8].into_iter().map(|x| x.into()).collect();

        let tree = SubProductTree::new_from_points(xs.as_slice());
        let interpolation_fast = Polynomial::lagrange_interpolation_with_tree(xs.as_slice(), ys.as_slice(), &tree);

        for (&x, &y) in xs.iter().zip(ys.iter()) {
            assert_eq!(interpolation_fast.eval(&x), y);
        }
