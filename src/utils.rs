...
use pairing::group::{ff::PrimeField, Curve};
...
pub fn is_subgroup_in_g1(point: &G1Projective) -> bool {
    //... slow
    if point.to_affine().is_torsion_free().unwrap_u8() == 0 {
        return false;
    }
    true
}

pub fn is_subgroup_in_g2(point: &G2Projective) -> bool {
    //... slow
    if point.to_affine().is_torsion_free().unwrap_u8() == 0 {
        return false;
    }
...
pub fn get_random_scalar_128() -> Scalar {
